class Rack::Attack
  ...
  # Throttle token refresh attempts (10 per minute per IP)
  throttle("admin_refresh/ip", limit: 10, period: 1.minute) do |req|
    req.ip if req.path == "/api/v1/admin/auth/refresh" && req.post?
  end
